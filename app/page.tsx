...
const CryptoCard = ({ name, symbol, icon, trend, isPositive, price, volume, marketCap }: { 
 name: string;
 symbol: string;
   ...
   <Card className="bg-gray-800/50 backdrop-blur border-indigo-500/20 hover:border-indigo-400/40 transition-all duration-300">
     <CardContent className="p-4">
       <div className="flex items-center">
         <div className="mr-3 w-10 h-10">{icon}</div>
         <div className="flex-1">
           ...
           <p className="text-lg font-bold text-white mt-1">${price}</p>
     ...
     <section className="py-16 md:py-20 px-4 bg-indigo-900/30 relative overflow-hidden">
       ...
       <div className="max-w-6xl mx-auto relative">
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
           {cryptoData?.length > 0 && cryptoData.map((coin: CryptoData) => (
             <CryptoCard 
               key={coin.id}
               name={coin.name}
               symbol={coin.symbol.toUpperCase()}
               icon={<Image src={coin.image} alt={coin.symbol} width={32} height={32} priority />}
               trend={`${coin.price_change_percentage_24h?.toFixed(1)}%`}
